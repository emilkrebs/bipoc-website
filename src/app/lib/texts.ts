...
export function createTextFromFile(fileName: string, directory: string, locale: string): Text {
...
export async function getTextsData(locale: string = "en") {
	try {
		...
		return fileNames.map(fileName => createTextFromFile(fileName, localeDirectory, locale));
	} catch (error) {
		...
		if (locale !== "en") {
			...
			return fileNames.map(fileName => createTextFromFile(fileName, englishDirectory, "en"));
		}
...
export async function getTextData(id: string, locale: string = "en") {
	try {
		...
		return createTextFromFile(`${id}.md`, path.join(TEXTS_DIRECTORY, locale), locale);
	} catch (error) {
		// Fallback to English if the locale file doesn't exist
		if (locale !== "en") {
			console.warn(`Text file for ${id} not found in locale ${locale}, falling back to English`);
			return createTextFromFile(`${id}.md`, path.join(TEXTS_DIRECTORY, "en"), "en");
		}
