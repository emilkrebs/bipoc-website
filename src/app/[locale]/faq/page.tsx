...
export default async function FaqPage({ params }: LocaleProps) {
    ...
    const accessibilitySpace = await getTextData(
        "faq/accessibility-space",
        locale,
    );
    const accessibilityLanguages = await getTextData(
        "faq/accessibility-languages",
        locale,
    );
    ...
    return (
        <main className="min-h-screen flex flex-col items-center justify-center bg-[url(/background_2.webp)] bg-fixed">
            ...
            <section
                id="faq"
                className="flex flex-col items-center w-full  py-16 px-4"
            >
                <div className="flex flex-col w-full max-w-4xl p-8 bg-white/90 rounded-2xl shadow-2xl border border-neutral-200">
                    <h2 className="text-4xl font-extrabold text-neutral-900 mb-2 tracking-tight">
            FAQ
                    </h2>
